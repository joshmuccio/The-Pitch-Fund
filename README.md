...
| **Sync Plasmic changes** | `./plasmic-sync.sh` ⭐ **(Recommended)** |
| **Alternative Plasmic sync** | `npm run plasmic:sync` |
...
├── plasmic-sync.sh             # Helper script for syncing Plasmic designs
...
| **`./plasmic-sync.sh` fails** | Ensure you're in project root with `plasmic.json` file |
