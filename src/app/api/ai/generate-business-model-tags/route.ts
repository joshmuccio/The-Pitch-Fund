...
import { validateTranscriptLength, generatePrompt, parseTagsFromResponse, COMMON_BUSINESS_MODEL_TAGS } from '@/lib/ai-helpers'
...
export async function POST(request: NextRequest) {
  try {
    ...
    const prompt = generatePrompt('business_model', transcript, COMMON_BUSINESS_MODEL_TAGS)
    ...
    if (!response) {
      Sentry.captureException(new Error('No business model tags response from OpenAI'), {
    ...
    if (tags.length === 0) {
      Sentry.captureException(new Error('Unable to parse business model tags from OpenAI response'), {
        tags: { route: 'ai/generate-business-model-tags', error_type: 'parse_failure' },
        extra: {
          raw_response: response
        }
      })
      return NextResponse.json(
        { error: 'Unable to parse business model tags from response. Please try again.' },
        { status: 500 }
      )
    }
