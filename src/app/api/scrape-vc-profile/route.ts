...
function extractVcDataFromHtml(html: string, profileUrl: string, sessionId: string) {
  ...
  if (profileImgContainer.length > 0) {
    ...
    if (srcset) {
      // Extract the S3 URL pattern from srcset (highest quality available) - support .jpg, .jpeg, .png, and .webp
      const s3UrlMatch = srcset.match(/https:\/\/s3\.us-west-1\.amazonaws\.com\/redwood-labs\/showpage\/uploads\/images\/[a-f0-9-]+\.(jpg|jpeg|png|webp)/g)
    ...
    if (!profileImageUrl && src) {
      // Check if src contains S3 URL pattern - support .jpg, .jpeg, .png, and .webp
      const s3UrlInSrc = src.match(/https:\/\/s3\.us-west-1\.amazonaws\.com\/redwood-labs\/showpage\/uploads\/images\/[a-f0-9-]+\.(jpg|jpeg|png|webp)/)
      if (s3UrlInSrc) {
